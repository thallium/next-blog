...
此题同样因为有先后顺序所以也不能用第一种思路，但思路和上题类似。 $[l, r]$ 中每个集合加入一个数就相当于在辅助数组中 $[l, r]$ 的位置上加 1，所以我们需要一个可以区间加的数据结构，最简单的就是树状数组啦。实现细节详见代码。
